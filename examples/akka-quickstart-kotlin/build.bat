...
set _MAIN_CLASS=com.example.AkkaQuickstartKt
set _MAIN_ARGS=
...
echo     %__BEG_O%compile%__END%     compile Kotlin source files
...
if exist "%__SOURCES_FILE%" del "%__SOURCES_FILE%" 1>NUL
set __N=0
for /f "delims=" %%i in ('dir /s /b "%_SOURCE_MAIN_DIR%\kotlin\*.kt" 2^>NUL') do (
    echo %%i >> "%__SOURCES_FILE%"
    set /a __N+=1
)
if %__N%==0 (
...
echo -language-version 1.5 -classpath "%_LIBS_CPATH:\=\\%%_CLASSES_DIR:\=\\%" -d "%_CLASSES_DIR:\=\\%" > "%__OPTS_FILE%"
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_KOTLIN_CMD%" -cp "%__CPATH%" %_MAIN_CLASS% 1>&2
) else if %_VERBOSE%== 1 ( echo Execute Kotlin program "%_MAIN_CLASS%" 1>&2
)
call "%_KOTLIN_CMD%" -cp "%__CPATH%" %_MAIN_CLASS%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute Kotlin program "%_MAIN_CLASS%" 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
