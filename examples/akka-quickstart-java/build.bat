...
set _MAIN_CLASS=com.example.AkkaQuickstart
set _MAIN_ARGS=
...
) else if %_VERBOSE%== 1 ( echo Execute Java program "%_MAIN_CLASS%" 1>&2
)
call "%_JAVA_CMD%" -cp "%__CPATH%" %_MAIN_CLASS%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute Java program "%_MAIN_CLASS%" 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
...
if exist "%__SOURCES_FILE%" del "%__SOURCES_FILE%" 1>NUL
set __N=0
for /f "delims=" %%i in ('dir /s /b "%_SOURCE_DIR%\test\java\*.java" 2^>NUL') do (
    echo %%i >> "%__SOURCES_FILE%"
    set /a __N+=1
)
if %__N%==0 (
