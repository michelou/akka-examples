...
set "_SOURCE_JAVA_DIR=%_ROOT_DIR%src\main\java"
set "_SOURCE_SCALA_DIR=%_ROOT_DIR%src\main\scala"
...
set _MAIN_CLASS=com.example.TestStrategies
set _MAIN_ARGS=
...
echo -deprecation -language:postfixOps -classpath "%__CPATH:\=\\%" -d "%_CLASSES_DIR:\=\\%" > "%__OPTS_FILE%"
...
) else if %_VERBOSE%==1 ( echo Execute Scala program "%_MAIN_CLASS%" 1>&2
)
call "%_SCALA_CMD%" -cp "%__CPATH%" %_MAIN_CLASS%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute Scala program "%_MAIN_CLASS%" 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
