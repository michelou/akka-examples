...
set "_SCALA2_CMD=%SCALA_HOME%\bin\scala.bat"
set "_SCALAC2_CMD=%SCALA_HOME%\bin\scalac.bat"

set _SCALA3_CMD=
if exist "%SCALA3_HOME%\bin\scalac.bat" (
...
if "%__ARG:~0,1%"=="-" (
    ...
    ) else if "%__ARG%"=="-scala3" ( set _SCALA3=1
...
set "_SCALA_CMD=%_SCALA2_CMD%"
set "_SCALAC_CMD=%_SCALAC2_CMD%"
if %_SCALA3%==1 if defined _SCALA3_CMD (
    set "_SCALA_CMD=%_SCALA3_CMD%"
    set "_SCALAC_CMD=%_SCALAC3_CMD%"
) else (
    echo %_WARNING_LABEL% Scala 3 installation not found 1>&2
    set _SCALA3=0
)
set _MAIN_CLASS=Main
set _MAIN_ARGS=
...
if %_DEBUG%==1 (
    ...
    if defined _SCALA3_CMD echo %_DEBUG_LABEL% Variables  : "SCALA3_HOME=%SCALA3_HOME%" 1>&2
