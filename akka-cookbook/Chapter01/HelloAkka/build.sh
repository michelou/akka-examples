...
libs_cpath() {
    ...
    jar_file=
    for f in $(find "$repo_dir/com/typesafe/akka" -name "akka-actor_2.13-2.8.1.jar" 2>/dev/null); do 
        jar_file="$f"
    done
	[[ -f "$jar_file" ]] && cpath="$cpath$(mixed_path $jar_file)$PSEP"
    ## https://mvnrepository.com/artifact/com.typesafe.akka/akka-actor-typed
    jar_file=
    for f in $(find "$repo_dir/com/typesafe/akka" -name "akka-actor-typed_2.13-2.8.1.jar" 2>/dev/null); do 
        jar_file="$f"
    done
	[[ -f "$jar_file" ]] && cpath="$cpath$(mixed_path $jar_file)$PSEP"
    echo "$cpath"
