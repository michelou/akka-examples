...
set _MAIN_CLASS=akka.examples.PingPongApp
set _MAIN_ARGS=
...
if %_DEBUG%==1 ( set __JAVAC_OPTS=-deprecation -Xdiags:verbose
) else ( set __JAVAC_OPTS=-deprecation
)
echo %__JAVAC_OPTS% -classpath "%_LIBS_CPATH:\=\\%%_CLASSES_DIR:\=\\%" -d "%_CLASSES_DIR:\=\\%" > "%__OPTS_FILE%"
