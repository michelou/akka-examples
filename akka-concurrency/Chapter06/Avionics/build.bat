...
set _MAIN_CLASS=zzz.akka.avionics.Avionics
set _MAIN_ARGS=
...
@rem output parameters: _LIBS_CPATH, _LIBS_CPATH_TEST
:libs_cpath
...
set "_LIBS_CPATH_TEST=%_CPATH_TEST%"
goto :eof
...
set "__CPATH=%_LIBS_CPATH%%_LIBS_CPATH_TEST%%_CLASSES_DIR%;%_TEST_CLASSES_DIR%"
set __SUITE_CLASS_NAME=zzz.akka.avionics.EventSourceSpec
@rem set __RUNNER_MAIN=org.scalatest.tools.Runner
@rem set __RUNNER_ARGS=-R "%_TEST_CLASSES_DIR%" -s "%__SUITE_CLASS_NAME%"
set __RUNNER_MAIN=org.scalatest.run
set __RUNNER_ARGS=%__SUITE_CLASS_NAME%

@rem see https://www.scalatest.org/user_guide/using_the_runner
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_SCALA_CMD%" -cp "%__CPATH%" %__RUNNER_MAIN% %__RUNNER_ARGS% 1>&2
) else if %_VERBOSE%==1 ( echo Execute ScalaTest suite "%__SUITE_CLASS_NAME%" 1>&2
)
call "%_SCALA_CMD%" -cp "%__CPATH%" %__RUNNER_MAIN% %__RUNNER_ARGS%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute ScalaTest suite "%__SUITE_CLASS_NAME%" 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
...
call :action_required "%__TIMESTAMP_FILE%" "%_SOURCE_DIR%\test\scala\*.scala"
if %_ACTION_REQUIRED%==0 goto :eof

call :test_compile_scala
if not %_EXITCODE%==0 goto :eof

for /f "delims=" %%f in ('dir /b /s "%_SOURCE_DIR%\test\resources\*" 2^>NUL') do (
   if %_DEBUG%==1 echo %_DEBUG_LABEL% copy /y "%%f" "%_TEST_CLASSES_DIR%\" 1>&2
   copy /y "%%f" "%_TEST_CLASSES_DIR%\" %_STDOUT_REDIRECT%
)
echo. > "%__TIMESTAMP_FILE%"
goto :eof
...
set "__CPATH=%_LIBS_CPATH%%_LIBS_CPATH_TEST%%_CLASSES_DIR%"
echo -deprecation -classpath "%__CPATH:\=\\%" -d "%_TEST_CLASSES_DIR:\=\\%" > "%__OPTS_FILE%"
